document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const errorMessages = {
      ...
      minlength: function (param) { return 'Minimum ' + param + ' characters required.'; },
      maxlength: function (param) { return 'Maximum ' + param + ' characters allowed.'; },
      tel: 'Please enter a valid phone number.'
    };

    function getErrorMessage(rule, param) {
      const msg = errorMessages[rule];
      return typeof msg === 'function' ? msg(param) : (msg || 'Invalid value.');
    }
    ...
    function validateField(field) {
      ...
      if (field.required || field.hasAttribute('required')) {
        rules.push({ rule: 'required', param: null });
      }

      if (type === 'email') {
        rules.push({ rule: 'email', param: null });
      }

      if (type === 'tel') {
        rules.push({ rule: 'tel', param: null });
      }
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        if (rule === 'required') {
          if (!validators.required(value)) {
            errorMessage = getErrorMessage('required', param);
            break;
          }
        } else {
          if (value.trim() === '') break;
          if (!validators[rule](value, param)) {
            errorMessage = getErrorMessage(rule, param);
            break;
          }
        }
      }
      ...
      if (errorMessage) {
        ...
        errorEl.setAttribute(ERROR_ATTR, errorMessage);
        return false;
      } else {
        ...
        errorEl.textContent = '';
        errorEl.removeAttribute(ERROR_ATTR);
        return true;
      }
    }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "צליל התנועה - כשאותיות הופכות למילים משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '3e4b2139-419b-4108-937d-abc4fef975c3',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '3e4b2139-419b-4108-937d-abc4fef975c3';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '3e4b2139-419b-4108-937d-abc4fef975c3';
